...
class PIDLOSS(nn.Module):
    ...
    def forward(self,
                ...
                **kwargs):
        # 获取 cls_score 的尺寸，并将其分配给 self.n_i（样本数量）和 self.n_c（类别数量）。
        # 同时，将标签和分类得分分别存储在 self.gt_classes 和 self.pred_class_logits 中
        self.n_i, self.n_c = cls_score.size()
        self.gt_classes = label
        self.pred_class_logits = cls_score

        # 将标签扩展为与 cls_score 相同尺寸的目标张量。
        # 每个样本的正确类别位置被设置为 1，其余位置为 0。
        ...
        # get_weight 函数计算正负样本的权重。然后，根据目标张量和权重计算最终的权重张量。
        self.pos_w, self.neg_w = self.get_weight(self.target)
        self.weight = self.pos_w * self.target + self.neg_w * (1 - self.target)
        # 根据 pidmask 和类别信息设置类别激活掩码。这个掩码用于调整权重张量。
        # 根据不同的类别（head, middle, tail）来激活或关闭某些类别的权重。
    ...
    def apply_class_activation(self):
        ...
        if self.class_activation:
            '''
            检查 pidmask 中是否不包含某个关键字，不包含则关闭控制器。
            '''
