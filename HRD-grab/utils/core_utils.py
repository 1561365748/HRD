...
def train_fl(datasets, cur, args, log=None):
    ...
    # 如果有多个 GPU，则将工作模型分配到不同的 GPU 上；否则，所有模型都分配到第一个 GPU 上。
    if device_counts > 1:
    ...
    # ----------给每个客户端初始化pid_losses----------
    g_pid_losses = [PIDLOSS(device=device_ids[i], pidmask=["head", "middle"], class_activation = False) for i in range(num_insti)] # 每一个client都有一个pidloss

    for idx in range(num_insti):
        g_pid_losses[idx].get_3shotclass(head_class=[], middle_class=[], tail_class=[0, 1])
    ...
    for epoch in range(args.max_epochs):        
        ...
        train_loop_fl(epoch, model, worker_models, train_loaders, worker_optims, 
                     args.n_classes, writer, g_pid_losses[idx], log)
...
def train(datasets, cur, args, log=None):
    ...
    g_pid_losses = PIDLOSS(device = device, pidmask=["head", "middle"], class_activation = False) 

    g_pid_losses.get_3shotclass(head_class=[], middle_class=[], tail_class=[0, 1])
    g_pid_losses.apply_3shot_mask()   
    # ------------------------------------------------

    for epoch in range(args.max_epochs):   
        #  训练模型
        # train_loop(epoch, model, train_loader, optimizer, args.n_classes, writer, loss_fn,log)
        train_loop(epoch, model, train_loader, optimizer, args.n_classes, writer, g_pid_losses, log)
